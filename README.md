...
5. The right panel is a scrollable betting board. Click the Ace or any numbered square (2–10) to stack chips; spots tighten into multiple rows on narrow screens so nothing overlaps. The footer’s **Clear Bets**, **Rebet**, and **Deal Hand** buttons stay pinned to the bottom alongside the centered chip selector.
...
* A neon-styled bankroll history chart pops out from the header graph icon. The canvas continually rescales to fit new data, trimming x-axis labels whenever space gets tight so you can review long sessions without horizontal scrolling.
